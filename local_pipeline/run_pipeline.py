...
def main():
    ...
    parser.add_argument("--pattern", type=str, default="budapest_2024", help="File pattern to match in Drive.")
    parser.add_argument("--folder", type=str, default="earth_engine_exports", help="Drive folder name.")
    parser.add_argument("--interactive", action="store_true", help="Force interactive OAuth authentication.")
    ...
    try:
        files = download_data(folder_name=args.folder, pattern=args.pattern, force_interactive=args.interactive)
        if not files:
    ...
    run_command([
        ...
        "--input_pattern", f"data/{args.pattern}-*.tif",
        "--output", merged_tif
    ])
