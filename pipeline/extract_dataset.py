...
class CustomOptions(PipelineOptions):
    ...
    def _add_argparse_args(cls, parser):
        ...
        parser.add_argument(
            '--bands',
            nargs='+',
            help='List of bands to select from the ImageCollection. If not specified, all bands will be used.'
        )
        parser.add_argument(
            '--scale',
            type=float,
            default=100,
            help='Scale factor for output Zarr.'
        )
        parser.add_argument(
            '--target_chunks',
            type=str,
...
def main(argv: list[str]) -> None:
    ...
    target_chunks = dict(source_chunks, **_parse_chunks_str(custom_options.target_chunks))

    credentials = ee.ServiceAccountCredentials(
        custom_options.ee_service_account_email,
    ...
    with beam.Pipeline(options=options) as root:
       _ = (
          root 
          | xbeam.DatasetToZarr(ds, custom_options.output, target_chunks)
       )
        # _ = (
        #     root
        #     | xbeam.DatasetTo(ds, source_chunks)
        #     | xbeam.Rechunk(
        #         ds.sizes,
        #         source_chunks,
        #         target_chunks,
        #         itemsize=itemsize,
        #     )
        #     | xbeam.ChunksToZarr(custom_options.output, template, target_chunks)
        # )
...
if __name__ == '__main__':
    import sys
    main(sys.argv)
    #app.run(main)  # Uncomment this line if you want to use absl app
    # python pipeline/extract_dataset.py --input_geojson gs://earth-embeddings-public/geojson/small_sample.geojson --output gs://earth-embeddings-public/raw_data/initial.zarr